...
curdir=`pwd -P`
if $( echo $curdir | grep --quiet 'p2shell/test-scripts' ); then
   ...
   for file in $allFiles; do
    ...
    echo "ERROR: You are not in the right directory because you should be in your p2shell/test-scripts directory. Exiting"
    exit
fi
